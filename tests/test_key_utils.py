...
class TestKeyParse:
    ...
    def test_valid(self, text: str, expected_parts: list) -> None:
        assert Key.parse(text).parts == expected_parts
...
class TestKeyGetter:
    class TestSingleStep:
        def test_attr(self) -> None:
            assert Key.parse("x")(ns(x=42)) == 42

        def test_positive_index(self) -> None:
            assert Key.parse("items[1]")(ns(items=[10, 20, 30])) == 20
    ...
    class TestChainedAttrs:
        def test_nested(self) -> None:
            assert Key.parse("a.b")(ns(a=ns(b=99))) == 99

        def test_deeply_nested(self) -> None:
            assert Key.parse("a.b.c")(ns(a=ns(b=ns(c="deep")))) == "deep"

    class TestIndexThenAttr:
        def test_positive(self) -> None:
            pts = [ns(x=1.0, y=2.0), ns(x=3.0, y=4.0)]
            assert Key.parse("pts[0].x")(ns(pts=pts)) == 1.0
    ...
    class TestLeadingBracket:
        def test_index(self) -> None:
            assert Key.parse("[1]")([10, 20, 30]) == 20
